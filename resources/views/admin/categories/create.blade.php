...
@section('content')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500"
                            placeholder="Deskripsi singkat tentang kategori">{{ old('description') }}</textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div class="flex items-center space-x-3">
                                <input type="color" id="color" name="color" value="{{ old('color', '#ef4444') }}"
                                    class="w-12 h-12 border-0 rounded cursor-pointer">
                                <input type="text" id="color_hex" value="{{ old('color', '#ef4444') }}"
                    ...
                    <div class="p-4 bg-gray-50 rounded-lg">
                        ...
                        <div class="flex items-center">
                            <div id="previewIcon"
                                class="w-10 h-10 rounded-lg flex items-center justify-center mr-3 bg-red-100">
                                <i id="previewIconClass" class="fas fa-calendar text-red-600"></i>
                            </div>
                            <div>
                                <div id="previewName" class="font-medium text-gray-900">Nama Kategori</div>
                                <div id="previewDesc" class="text-sm text-gray-500">Deskripsi akan muncul di sini</div>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        document.getElementById('description').addEventListener('input', function() {
            document.getElementById('previewDesc').textContent = this.value || 'Deskripsi akan muncul di sini';
        });

        document.getElementById('icon').addEventListener('input', function() {
            const previewIcon = document.getElementById('previewIconClass');
            previewIcon.className = this.value || 'fas fa-calendar';
        });
